...
ALLOWED_EMAILS = set(
    addr.strip().lower() for addr in config.get('filtering', 'allowed_emails', fallback='').split(',') if addr.strip())
...
def normalize_body(html_or_text):
    ...
    try:
        ...
        # On garde les sauts de ligne originaux et on nettoie juste les espaces superflus par ligne
        text = soup.get_text(separator='\n')
        return "\n".join(line.strip() for line in text.splitlines() if line.strip())
    except:
        return str(html_or_text).strip()
...
def process_mbox_file(mbox_path, folder_path, mailbox_id, session):
    ...
    print(f"      📂 Analyse dossier : {folder_path}", flush=True)
    ...
    for msg in mbox:
        try:
            ...
            if not ((sender_email in ALLOWED_EMAILS and any(e in ALLOWED_EMAILS for e in all_rec_list)) or
                    (not sender_email and any(e in ALLOWED_EMAILS for e in all_rec_list)) or
                    (sender_email in ALLOWED_EMAILS and not all_rec_list)):
                continue

            # --- Date et Corps ---
            try:
                mail_date = parsedate_to_datetime(msg['date'])
                if mail_date.year < 1000 or mail_date.year > 9999: mail_date = datetime.now()
            except:
            ...
            # 3. DB Insertion
            res_email = session.execute(text("SELECT id FROM email WHERE message_fingerprint = :f"),
            ...
            if not res_email:
                try:
                    ...
                    for email_addr in all_rec_list:
                        ...
                        session.execute(
                            text("INSERT IGNORE INTO email_recipient (email_id, identity_id) VALUES (:eid, :rid)"),
                            {"eid": email_id, "rid": rid})
            ...
            if email_id:
                session.execute(text(
                    "INSERT IGNORE INTO email_mailbox (email_id, mailbox_id, folder_path) VALUES (:eid, :mid, :fp)"),
                    {"eid": email_id, "mid": mailbox_id, "fp": folder_path})

            messages_indexed += 1
            if messages_indexed % 500 == 0:
                session.commit()
                print(f"      [{mbox_path.name}] {messages_indexed} items processed...", flush=True)
...
def worker_process_pst(pst_path):
    ...
    try:
        print(f"🚀 Processing: {pst_path.name}", flush=True)
        ...
        if res:
            ...
            res_ins = session.execute(
                text("INSERT INTO mailbox (owner_identifier, pst_filename) VALUES (:o, :f)"),
                {"o": pst_path.stem, "f": abs_pst})
            mailbox_id = res_ins.lastrowid
        session.commit()

        print(f"   🛠  Extraction PST (readpst) pour {pst_path.name}...", flush=True)
        subprocess.run(["readpst", "-S", "-M", "-e", "-r", "-o", str(temp_dir), abs_pst], check=False,
                       capture_output=True)

        total = sum(process_mbox_file(f, str(f.parent.relative_to(temp_dir)), mailbox_id, session)
                    for f in temp_dir.rglob("*") if f.is_file() and not f.name.startswith('.'))
        print(f"✅ Finished {pst_path.name}: {total} emails.", flush=True)
    finally:
        if temp_dir.exists(): shutil.rmtree(temp_dir)
        session.close()
...
def main(source_dir):
    ...
    if not pst_files:
        print("❌ Aucun fichier PST trouvé dans le répertoire spécifié.", flush=True)
        return

    print(f"🔍 {len(pst_files)} PST identifiés. Starting pipeline with {multiprocessing.cpu_count()} workers.",
          flush=True)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[0]} <directory_with_psts>", flush=True)
        sys.exit(1)
    main(sys.argv[1])
